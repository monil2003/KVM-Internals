...
int run_vm(struct vm *vm1, struct vm *vm2, struct vcpu *vcpu1, struct vcpu *vcpu2, size_t sz)
{
    ...
    uint32_t *consumer_arr = NULL, *producer_arr = NULL;
    uint32_t *consumer_prodp = NULL, *producer_prodp = NULL;
    uint32_t *consumer_consp = NULL, *producer_consp = NULL;

    uint32_t buffer[20] = {0};
    int32_t prod_p = -1, cons_p = -1;

    int32_t sched_order_index = 0;
    int32_t is_setup_complete = 0;

    for (; sched_order_index < 100;){
        if (is_setup_complete){
            printf("HYPVSR: [");
            if (prod_p != -1){
                for (int i = (cons_p == -1 ? 0 : cons_p); i != prod_p; i = (i + 1) % 20){
                    printf("%d ", buffer[i]);
                }
                printf("%d", buffer[prod_p]);
            }
            printf("]\n");
            if (sched_order[sched_order_index++] == '1'){
				vm = vm1;
				vcpu = vcpu1;
			}else{
				vm = vm2;
        ...
        switch (vcpu->kvm_run->exit_reason){
			...
			case KVM_EXIT_IO:
				if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT && vcpu->kvm_run->io.port == 0x10){
					if (vm == vm1){
						uint32_t gva = *((uint32_t *)((char *)vcpu->kvm_run + vcpu->kvm_run->io.data_offset));
						uint32_t *hva = (uint32_t *)((char *)vm->mem + gva);
						producer_arr = hva;
						continue;
					}
					else{
				...
				else if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT && vcpu->kvm_run->io.port == 0x11){
					if (vm == vm1){
						uint32_t gva = *((uint32_t *)((char *)vcpu->kvm_run + vcpu->kvm_run->io.data_offset));
						uint32_t *hva = (uint32_t *)((char *)vm->mem + gva);
						producer_prodp = hva;
				...
				else if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT && vcpu->kvm_run->io.port == 0x12){
					if (vm == vm1){
						uint32_t gva = *((uint32_t *)((char *)vcpu->kvm_run + vcpu->kvm_run->io.data_offset));
						uint32_t *hva = (uint32_t *)((char *)vm->mem + gva);
						producer_consp = hva;
				...
				else if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT && vcpu->kvm_run->io.port == 0x14){
					int32_t new_prod_p = *((int32_t *)((char *)vcpu->kvm_run + vcpu->kvm_run->io.data_offset));
					if (new_prod_p == prod_p){
						printf("VMFD: %d Produced %d Values: \n", vm->vm_fd, 0);
					}
					else{
						int cnt = 0;
						for (int32_t i = (prod_p + 1) % 20; i != new_prod_p; i = (i + 1) % 20){
							buffer[i] = producer_arr[i];
							consumer_arr[i] = producer_arr[i];
							cnt++;
						}
						buffer[new_prod_p] = producer_arr[new_prod_p];
						consumer_arr[new_prod_p] = producer_arr[new_prod_p];
						cnt++;

						printf("VMFD: %d Produced %d Values: ", vm->vm_fd, cnt);
						for (int32_t i = (prod_p + 1) % 20; i != new_prod_p; i = (i + 1) % 20){
							printf("%d ", buffer[i]);
						}
						printf("%d ", buffer[new_prod_p]);
						printf("\n");
						prod_p = new_prod_p;
						*consumer_prodp = prod_p;
						*producer_prodp = prod_p;
					}
					continue;
				}
				else if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT && vcpu->kvm_run->io.port == 0x15){
					int32_t new_cons_p = *((int32_t *)((char *)vcpu->kvm_run + vcpu->kvm_run->io.data_offset));
					int cnt = 0;
					if (new_cons_p != -1 && cons_p == -1)
						cons_p = 0;
					for (int32_t i = cons_p; i != new_cons_p; i = (i + 1) % 20){
						buffer[i] = consumer_arr[i];
						// consumer_arr[i] = buffer[i]; 
						producer_arr[i] = consumer_arr[i];
						cnt++;
					}
					printf("VMFD: %d Consumed %d Values: ", vm->vm_fd, cnt);
					for (int32_t i = cons_p; i != new_cons_p; i = (i + 1) % 20){
						printf("%d ", buffer[i]);
					}
					printf("\n");
					cons_p = new_cons_p;
					*producer_consp = cons_p;
					*consumer_consp = cons_p;
					continue;
				}
				else{
